// =============================================
// COOPs 議事録AI v3.0 — Code.gs
// =============================================

// ↓ GeminiのAPIキーをここに貼り付けてください（改行なし・1行で）
const GEMINI_API_KEY = 'AIzaSyAMexlCd4FKuQuL1eyiLYhUMDopnuuECxc';
const GEMINI_MODEL   = 'gemini-3.1-flash-lite-preview';
const CLEAN_KEY      = GEMINI_API_KEY.replace(/[\s\r\n]+/g, '').trim();
...
function getMasterData() {
  try {
    ...
    return JSON.stringify({ employees: empData, departments, meetingTypes });
  } catch(e) {
...
// アジェンダ生成
// =============================================
function generateAgenda(paramsJson) {
  try {
...
// アジェンダ保存
// =============================================
function saveAgenda(dataJson) {
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(30000); // 30秒間ロックを待機
    initSheets();
...
function getAgendaList(dept, meetingType) {
  try {
    ...
    let list = sheet.getDataRange().getValues().slice(1)
      .filter(r => r[COL.record_id - 1] && String(r[COL.status - 1]) !== '削除済')
      .map(r => {
    ...
    if (meetingType && meetingType !== 'all') list = list.filter(r => r.meetingType === meetingType);
...
// 議事録生成
// =============================================
function generateMinutes(inputText, audioDataJson, meetingInfoJson) {
  try {
    const info = JSON.parse(meetingInfoJson || '{}');
...
【出力形式】以下のJSONのみで回答（コードブロック不要）:
{
  ...
  "facilitator_feedback": "AIファシリテーターとしての評価（良かった点2つ・改善提案2つ）"
    ...
    let audioData = null;
    if (audioDataJson) {
      try { audioData = JSON.parse(audioDataJson); } catch(_) {}
    }
    ...
    if (d.agendaRecordId) {
      ...
      for (let i = 1; i < values.length; i++) {
        if (String(values[i][COL.record_id - 1]) === String(d.agendaRecordId)) {
          ...
          const updates = [
            [COL.status,             '議事録完了'],
            [COL.minutes_timestamp,  now],
            [COL.input_text,         clipText(d.inputText)],
            [COL.summary,            clipText(d.summary)],
            [COL.agenda_items,       clipText(d.agenda_items)],
            [COL.key_discussions,    clipText(d.key_discussions)],
            [COL.action_plans,       clipText(d.action_plans)],
            [COL.culture_notes,      clipText(d.culture_notes)],
            [COL.next_agenda,        clipText(d.next_agenda)],
            [COL.facilitator_feedback, clipText(d.facilitator_feedback)]
          ];
          updates.forEach(([col, val]) => sheet.getRange(r, col).setValue(val));
          return JSON.stringify({ success: true, message: 'アジェンダに議事録を紐づけて保存しました' });
        }
      }
      return JSON.stringify({ error: '対応するアジェンダが見つかりませんでした。' });
    } else {
      // 新規行として保存
      const recordId = Utilities.getUuid();
      const row = new Array(19).fill('');
      ...
      sheet.appendRow(row);
      return JSON.stringify({ success: true, message: '議事録を新規保存しました', recordId });
    }
  } catch(e) {
    return JSON.stringify({ error: e.message });
  } finally {
...
// 履歴一覧取得（統合テーブル）
// =============================================
function getHistoryList(dept, meetingType) {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOG_SHEET);
    if (!sheet) return JSON.stringify([]);
    ...
    let list = sheet.getDataRange().getValues().slice(1)
      ...
      .map(r => ({
        ...
        meetingDate:         String(r[COL.meeting_date - 1]),
        dept:                String(r[COL.dept - 1]),
        meetingType:         String(r[COL.meeting_type - 1]),
        participants:        String(r[COL.participants - 1]),
        clientName:          String(r[COL.client_name - 1]),
...
function deleteLog(recordId) {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOG_SHEET);
    if (!sheet) return JSON.stringify({ error: 'ログシートが見つかりません' });
    ...
    return JSON.stringify({ error: '該当レコードが見つかりません' });
  } catch(e) {
    return JSON.stringify({ error: e.message });
  }
}
...
function exportToGoogleDoc(recordId) {
  try {
    ...
    if (!row) return JSON.stringify({ error: '該当レコードが見つかりません' });
    ...
    doc.saveAndClose();
    return JSON.stringify({ success: true, url: doc.getUrl(), title });
  } catch(e) {
    return JSON.stringify({ error: e.message });
  }
}
...
function callGemini(prompt, audioData) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${CLEAN_KEY}`;
...
function extractJson(text) {
  ...
  if (!match) return JSON.stringify({ error: 'AIの出力を解析できませんでした' });
  return match[0];
}
...
// =============================================
// 初回権限承認用関数（GASエディタでこれを選択して実行してください）
// =============================================
function setupPermissions() {
  // DriveとDocsへのアクセス権限を強制的に要求するためのダミー処理
  DriveApp.getFiles();
}

// =============================================
// Drive 連携 (フロントエンドからの独自検索・抽出)
// =============================================
...
function extractDriveText(fileId, mimeType) {
  try {
    ...
    if (mimeType === MimeType.GOOGLE_DOCS) {
      ...
      return JSON.stringify({ error: 'この形式（' + mimeType + '）のテキスト抽出には対応していません。' });
    }
